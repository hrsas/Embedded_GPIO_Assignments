...
void display()
{
  ...
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
      send_4(i,i,i,i,0, 0,0,display_E[i-1]);
    }
  }
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
      send_4(i,i,i,i,0,0, display_E[i-1], display_T[i-1]);
    }
  }
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
      send_4(i,i,i,i,0,display_E[i-1],display_T[i-1],display_A[i-1]);
    }
  }
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
  ...
  for (int i = 1; i <= 8; i++) {
    for (int j = 1; j <= 8; j++) {
      if(i-1+j < 8){
        send_4(j, j, j, j, display_E[i-1+j], display_T[i-1+j],display_A[i-1+j],display_L[i-1+j]);
      }
      else{
        send_4(j, j, j, j, 0,0,0,0);
      }
    }
    my_delay(100000);
  }
