...
void display()
{
  ...
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
      send_4(i,i,i,i,display_L[i-1],0, 0,0);
    }
  }
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
      send_4(i,i,i,i,display_A[i-1], display_L[i-1],0, 0);
    }
  }
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
      send_4(i,i,i,i,display_T[i-1],display_A[i-1],display_L[i-1],0);
    }
  }
  for(volatile long j = 0; j<1000; j++){
    for(volatile long i = 1; i<=8; i++){
  ...
  for (int i = 1; i <= 8; i++) {
    for (int j = 1; j <= 8; j++) {
      int idx = j - 1 - i; 
      if (idx >= 0) {
        send_4(j, j, j, j,display_E[idx],display_T[idx],display_A[idx],display_L[idx]);
      } else {
